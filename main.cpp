...
*/
  ...
  sInt sAppMain( sInt argc, sChar **argv )
  {
    ...
    //sSystem->PrintF("WARNING: THIS VERSION IGNORES IMPORT LOAD FAILURES DURING DEPACKING!\n         YOU NEED TO HANDLE THEM IN YOUR PROGRAM!\n         (LoadLibrary is your friend)\n\n");
    ...
    else
    {
      ...
      out = pack.Pack( in, inSize, outSize, gotMap ? &info : 0, packerCallback, 0 );
      if ( out )
      {
        ...
        if ( gotMap )
        {
          ...
          if ( i > 0 )
            sCopyString( fileBuf + i, ".kkp", 260 - i );
          else
            sAppendString( fileBuf, ".kkp", 260 );
          info.DumpPackData( fileBuf, in, inSize );
